// Hace llamado a la tabla y aplica paginacion -->
  ...
  $(document).ready(function(){
    ...
    tabla_fisica = $('#tabla-afi').DataTable({
      scrollX: true,
      autoWidth: true,
      responsive: false,
      scrollY: 450,
      ...
      initComplete: function() {
        ...
        $('#tabla-afi_filter input').off();

        // 2. Agregar tu propio control
        $('#tabla-afi_filter input').on('input', function () {
    ...
    $(document).on('click', '.js-activar-excel', function (e) {
        e.preventDefault();
        tabla_fisica.button('.buttons-excel').trigger();
    });
...
$('#tabla-afi').on('click', '.btnEditar', function () {

    wizardMode = 'edit';
    editidpaceven = $(this).data('idpacienteevento');

    $('#myModalLabel').text('Editar Fisico');
...
function cargarDatosColaborador(editidpaceven) {
    $.ajax({
        ...
        success: function (data) {
            ...
            // Paso 2
            $('#asistencia').val(data.acudiofis);
            $('#observaciones').val(data.obs_fis);
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (resp.success) {
                ...
                if ($.fn.DataTable.isDataTable('#tabla-afi')) {
                    $('#tabla-afi').DataTable().ajax.reload(null, false);
                }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(response.success){
                ...
                tabla_fisica.ajax.reload(null, false);

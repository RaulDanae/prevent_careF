// Hace llamado a la tabla y aplica paginacion -->
  ...
  $(document).ready(function(){
    ...
    tabla_auditiva = $('#tabla-cau').DataTable({
      scrollX: true,
      autoWidth: true,
      responsive: false,
      scrollY: 450,
      ...
      initComplete: function() {
        ...
        $('#tabla-cau_filter input').off();

        // 2. Agregar tu propio control
        $('#tabla-cau_filter input').on('input', function () {
    ...
    $(document).on('click', '.js-activar-excel', function (e) {
        e.preventDefault();
        tabla_auditiva.button('.buttons-excel').trigger();
    });
...
$('#tabla-cau').on('click', '.btnEditar', function () {

    wizardMode = 'edit';
    editidpaceven = $(this).data('idpacienteevento');

    $('#myModalLabel').text('Editar Auditivo');
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (resp.success) {
                ...
                if ($.fn.DataTable.isDataTable('#tabla-cau')) {
                    $('#tabla-cau').DataTable().ajax.reload(null, false);
                }

// Hace llamado a la tabla y aplica paginacion -->
  ...
  $(document).ready(function(){
    ...
    tabla_vitales = $('#tabla-vit').DataTable({
      scrollX: true,
      autoWidth: true,
      responsive: false,
      scrollY: 450,
...
function buildSummary() {
    const summary = [
        { label: 'Pres Sistolica', value: $('#sistolica').val() },
        { label: 'Pres Distolica', value: $('#diastolica').val() },
        { label: 'Frec Cardiaca', value: $('#cardiaca').val() },
        { label: 'Oxigenacion', value: $('#oxigenacion').val() },
...
function cargarDatosColaborador(CURP) {
    $.ajax({
        ...
        success: function (data) {
            ...
            // Paso 2
            $('#sistolica').val(data.psistolica);
            $('#diastolica').val(data.pdiastolica);
            $('#cardiaca').val(data.fcardiaca);
            $('#oxigenacion').val(data.oxigenacion);
            $('#observaciones').val(data.obs_vital);
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (resp.success) {
                ...
                if ($.fn.DataTable.isDataTable('#tabla-vit')) {
                    $('#tabla-vit').DataTable().ajax.reload(null, false);
                }

// Hace llamado a la tabla y aplica paginacion -->
  let tabla_nutricional

  $(document).ready(function(){

    tabla_nutricional = $('#tabla-snu').DataTable({
      scrollX: true,
      autoWidth: true,
      responsive: false,
      scrollY: 450,
      ...
      initComplete: function() {
        ...
        $('#tabla-snu_filter input').off();

        // 2. Agregar tu propio control
        $('#tabla-snu_filter input').on('input', function () {
    ...
    $('#btn_search').click(function() {
      tabla_nutricional.draw();
    })

    // Boton Descargar
    $(document).on('click', '.js-activar-excel', function (e) {
        e.preventDefault();
        tabla_nutricional.button('.buttons-excel').trigger();
    });
...
$('#tabla-snu').on('click', '.btnEditar', function () {

    wizardMode = 'edit';
    editidpaceven = $(this).data('idpacienteevento');

    $('#myModalLabel').text('Editar Nutricion');
...
function cargarDatosColaborador(editidpaceven) {
    $.ajax({
        ...
        success: function (data) {
            ...
            // Paso 2
            $('#asistencia').val(data.acudiosn);
            $('#observaciones').val(data.obs_nutr);
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (resp.success) {
                ...
                if ($.fn.DataTable.isDataTable('#tabla-snu')) {
                    $('#tabla-snu').DataTable().ajax.reload(null, false);
                }

// Hace llamado a la tabla y aplica paginacion -->
  ...
  $(document).ready(function(){
    ...
    tabla_muestra = $('#tabla-tdm').DataTable({
      scrollX: true,
      autoWidth: true,
      responsive: false,
      scrollY: 450,
      ...
      initComplete: function() {
        ...
        $('#tabla-tdm_filter input').off();

        // 2. Agregar tu propio control
        $('#tabla-tdm_filter input').on('input', function () {
    ...
    $(document).on('click', '.js-activar-excel', function (e) {
        e.preventDefault();
        tabla_muestra.button('.buttons-excel').trigger();
    });
...
function buildSummary() {
    const summary = [
        { label: 'Asistencia', value: $('#asistencia').val() },
...
$('#tabla-tdm').on('click', '.btnEditar', function () {

    wizardMode = 'edit';
    editidpaceven = $(this).data('idpacienteevento');

    $('#myModalLabel').text('Editar Toma Muestra');
...
function cargarDatosColaborador(editidpaceven) {
    $.ajax({
        ...
        success: function (data) {
            ...
            // Paso 2
            $('#asistencia').val(data.acudiotm);
            $('#observaciones').val(data.obs_mues);
    ...
    $.ajax({
        ...
        success: function (resp) {
            ...
            if (resp.success) {
                ...
                if ($.fn.DataTable.isDataTable('#tabla-tdm')) {
                    $('#tabla-tdm').DataTable().ajax.reload(null, false);
                }
    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if(response.success){
                ...
                tabla_muestra.ajax.reload(null, false);

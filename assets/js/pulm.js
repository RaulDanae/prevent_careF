// Hace llamado a la tabla y aplica paginacion -->
  ...
  $(document).ready(function(){
    ...
    tabla_pulmonar = $('#tabla-cpu').DataTable({
      ...
      "ajax": {
        url: BASE_URL + "/partials/rellcpu.php",
        type: "POST",
        },
      ...
      initComplete: function() {
        ...
        $('#tabla-cpu_filter input').off();

        // 2. Agregar tu propio control
        $('#tabla-cpu_filter input').on('input', function () {
...
$('#tabla-cpu').on('click', '.btnEditar', function () {
    ...
    $('#myModalLabel').text('Editar Pulmonar');
...
function cargarDatosColaborador(editidpaceven) {
    $.ajax({
        url: BASE_URL + '/config/get_cpu.php',
        type: 'POST',
        dataType: 'json',
...
$('#formWizard').on('submit', function (e) {
    ...
    $.ajax({
        url: BASE_URL + '/save/update_pulmonar.php',
        type: 'POST',
        data: $(this).serialize(),
        dataType: 'json',
